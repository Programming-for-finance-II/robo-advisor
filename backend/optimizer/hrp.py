...
class OptimizationResult(TypedDict):
    """
    Result dictionary returned by the portfolio optimization function.
    
    Attributes:
        algorithm: The optimization algorithm used ("HRP", "MV", or "BL").
        weights: Dictionary mapping ticker symbols to their portfolio weights.
        ...
        sharpe_ratio: Sharpe ratio (expected return / volatility).
        risk_contributions: Dictionary mapping tickers to their risk contribution.
        optimizer_version: Version string of the optimizer implementation.
        ...
        fallback_tickers_applied: List of fallback tickers that were substituted.
    """
...
def compute_log_returns(prices: pd.DataFrame) -> pd.DataFrame:
    ...
    Calculates natural logarithm of price ratios: ln(P_t / P_{t-1}).
    ...
    Returns:
        DataFrame of log returns with NaN rows dropped.
        
    Raises:
        AssertionError: If prices are empty, have <2 assets, contain all-NaN columns,
            or result in <60 observations after computing returns.
    """
