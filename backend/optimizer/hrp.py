...
class OptimizationResult(TypedDict):
    """
    Result dictionary returned by the optimize() function.
    ...
    Attributes:
        ...
        expected_volatility: Annualized volatility (standard deviation) of the portfolio.
        sharpe_ratio: Sharpe ratio (expected_return / expected_volatility).
        ...
        solver_status: Status of the optimization solver ('optimal', 'clipped', or 'fallback_erc').
        ucits_tickers_used: List of UCITS-compliant tickers used in the optimization.
        fallback_tickers_applied: List of fallback tickers applied if primary assets were unavailable.
    """
...
def compute_log_returns(prices: pd.DataFrame) -> pd.DataFrame:
    ...
    Calculates log(P_t / P_{t-1}) for each asset and drops the first NaN row.
    
    Args:
        prices: DataFrame of adjusted close prices (rows=dates, cols=tickers).
    
    Returns:
        DataFrame of log returns with the same columns as prices and one fewer row.
