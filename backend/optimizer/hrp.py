...
class OptimizationResult(TypedDict):
    ...
    Attributes:
        ...
        weights: Dictionary mapping ticker symbols to portfolio weights (sum=1).
        expected_return: Annualized expected portfolio return (252 trading days).
        expected_volatility: Annualized portfolio volatility (standard deviation).
        sharpe_ratio: Risk-adjusted return metric (expected_return / expected_volatility).
        risk_contributions: Dictionary mapping tickers to their marginal risk contributions.
        optimizer_version: Version string of the optimizer library.
        solver_status: Status indicating whether constraints were applied ("optimal", "clipped", or "fallback_erc").
        ucits_tickers_used: List of UCITS-compliant tickers used in optimization.
        fallback_tickers_applied: List of fallback tickers applied if primary data unavailable.
    """
...
def compute_log_returns(prices: pd.DataFrame) -> pd.DataFrame:
    ...
    Computes logarithmic returns as log(P_t / P_{t-1}) and drops the first row (NaN).
    Log returns are time-additive and approximate simple returns for small changes.
